...
export const SchoolInfo = ({ edit, role }) => {
  ...
  const fetchSchoolData = async () => {
    try {
      ...
      setRoutes(
        schoolData.data.routes.map((route) => ({
          ...route,
          stops: route.stops.map((stop) => ({
            ...stop,
            arrivalIndex: parseInt(stop.arrivalIndex),
          })),
        }))
      );
    ...
    if (!name || name.trim().length === 0) {
      ...
      try {
        ...
        for (var i = 0; i < routes.length; i++) {
          await saveRoute({
            ...routes[i],
            students: routes[i].students.map((student) => ({
              ...student,
              inRangeStops: student.inRangeStops.map((stop) =>
                routes[i].stops.find((s) => s.uid === stop.uid)
              ),
            })),
          });
        }
  ...
  const calculateRoutes = async (newLat, newLng) => {
    try {
      ...
      for (
        ...
        const sorted = [
          ...routes[selectedRoute].stops
            .slice(0)
            .sort((a, b) => b.arrivalIndex - a.arrivalIndex),
        ];
        var newPolylinesEncoded = [];
        for (var j = 0; j < sorted.length / 26; j++) {
          const request = {
            origin: {
              lat: j === 0 ? newLat : parseFloat(sorted[j * 26 - 1].latitude),
              lng: j === 0 ? newLng : parseFloat(sorted[j * 26 - 1].longitude),
            },
            destination: {
              lat: parseFloat(
            ...
            waypoints: sorted
              .filter(
                (stop, index) =>
                  index >= j * 26 &&
                  index < Math.min(sorted.length - 1, j * 26 + 25)
              )
              .map((stop) => ({
                location: {
          ...
          if (result.status === "OK") {
            ...
            for (var i = 0; i < result.routes[0].legs.length; i++) {
              if (i === 0 && j === 0) {
                sorted[0].pickupTime = subtractDriveTime(
                  arrivalTime,
                  Math.trunc(result.routes[0].legs[0].duration.value / 60)
                );
                sorted[0].dropoffTime = addDriveTime(
                  departureTime,
                  Math.trunc(result.routes[0].legs[0].duration.value / 60)
                );
              } else {
                sorted[j * 26 + i].pickupTime = subtractDriveTime(
                  sorted[j * 26 + i - 1].pickupTime,
                  Math.trunc(result.routes[0].legs[i].duration.value / 60)
                );
                sorted[j * 26 + i].dropoffTime = addDriveTime(
                  sorted[j * 26 + i - 1].dropoffTime,
                  Math.trunc(result.routes[0].legs[i].duration.value / 60)
                );
              }
            }
            ...
            onChange={(e) => {
              ...
              setRoutes(
                routes.map((route) => ({
                  ...
                  stops: route.stops.map((stop) => ({
                    ...stop,
                    dropoffTime: findNewDropOffTime(
                      e.target.value,
                      departureTime,
                      stop.dropoffTime
                    ),
