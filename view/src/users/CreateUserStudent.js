...
export const CreateUser = () => {
  ...
  async function handleCreateStudent(e) {
    ...
    try {
      ...
      let create_student_response = await saveStudent({
        ...form_results,
        parentUser: madeUser,
      });
    } catch (error) {
  ...
  async function handleStudentCreateFormButton(e) {
    ...
    if (!firstNameStudent || !lastNameStudent) {
      ...
      if (makeStudentForUser) {
        ...
        alert(
          "Successfully Added Student Info to User. Note: Students are created only when the user form is submitted. To create a student independently, select Create New Student"
        );
      } else {
        // make new student
        setStudents((arr) => [...arr, newStudent]);
  ...
  async function handleUserCreateFormButton(e) {
    if (!firstNameUser || !lastNameUser) {
      ...
      if (makeUserForStudent) {
        ...
        alert(
          "Successfully Added User Info to Student! Note: Users are created only when this form is submitted. To create a user independently, select 'Create New User'"
        );
      } else {
        // make new user
        handleCreateUser(e);
          ...
          <form>
            ...
            <label className="input">
              ...
              <input
                ...
                onChange={(e) => {
                  setFilterValueSchool(e.target.value);
                  setSelectedSchool(false);
                  setSelectedRoute(false);
                  setRouteFilter("");
                }}
            ...
            <p>
              {" "}
              {selectedSchool && (!school.routes || school.routes.length === 0)
                ? " There Are No Routes Available For The Selected School. Please Create One In the Routes Tab."
                : ""}{" "}
            </p>
            <p>
              {" "}
              {!selectedSchool
                ? "You must select a school before you will be able to assign a route to this student."
                : ""}{" "}
            </p>
            {selectedSchool && school.routes && school.routes.length > 0 && (
              <label className="input">
            ...
            {selectedSchool && !selectedRoute && (
              <div className="route-list">
                {school.routes
                  .filter((route) =>
                    route.name.toLowerCase().includes(routeFilter.toLowerCase())
                  )
                  .splice(0, 10)
                  .map((route) => (
                    <li>
            ...
            {!makeStudentForUser && (
              <div>
                ...
                <h4>
                  {" "}
                  Associated User = {selectedUser ? "" + email : "None"}{" "}
                </h4>
              </div>
            )}

            {!makeStudentForUser &&
              !makeUserForStudent && ( //search for existing user
                <div>
                  <label className="input">
                    <p> User Search: </p>
                    <input
            ...
            <div>
              <button
                ...
                type="button"
                onClick={(e) => {
                  handleStudentCreateFormButton(e);
                }}
              >
                {makeStudentForUser ? "Add Student" : "Submit"}{" "}
              </button>
            </div>
          </form>
        </div>
        ...
        <div id="user_create_form">
          ...
          <form on>
            ...
            <label className="input">
              ...
              />
              <button onClick={(e) => checkMap(e)}>
                {" "}
                {addressValid ? "Address Valid!" : "Validate"}{" "}
              </button>
            </label>
      ...
      <div id="user_create_map">
        ...
        {showMap && (
          ...
          >
            ...
            >
              <Marker text="Your Address" lat={lat} lng={lng} />
            </GoogleMapReact>
          </div>
        )}
      </div>
