...
export const EditManager = ({complete, setComplete, message, editableFileData, setEditableFileData, columns, editableColumns, checkRow, checkCell, selectedIndex, setSelectedIndex, resetErrorData, resetWarningData}) => {
    ...
    const searchFileAgainstNewValue = (old_row, new_index, col, val) => { //(row, rowIndex, columnId, value)
        ...
        if (col === 'email') {
            ...
            if (old_row.email && val && old_row.email.toLowerCase().trim() === val.toLowerCase().trim()) {
    ...
    const searchFileData = (row, row_ind) => {
      ...
      let matching_email_indexes = editableFileData.reduce((r, n, i) => {
        (n.email && row.email && (n.index !== row_ind) && n.email.toLowerCase().trim() === row.email.toLowerCase().trim()) && r.push(i);
        return r;
      }, []);

      let matching_name_indexes = editableFileData.reduce((r, n, i) => {
        (n.name && row.name && (n.index !== row_ind) && n.name.trim() === row.name.trim()) && r.push(i);
        return r;
      }, []);
      ...
      if (matching_email_indexes.length > 0) {
          if (matching_email_indexes.length > 1) {
            ret.push([50, "Email Matches Rows " + matching_email_indexes.join(",")])
          }
          else {
        ...
        setEditableFileData(old =>
          old.map((row, index) => {
            if (index === rowIndex) {
                ...
                let a= {...copy, ['hint_uids']: [error_uid], ['warning_uids']: [warning_uid], ['error_code']: row_errors, ['loc']: loc_values, ['address']: address[0], ['valid']: v , ['duplicate']: dup } 
